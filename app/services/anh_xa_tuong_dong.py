...
def tim_kiem_tuong_dong(chuoi_can_tim, danh_sach_du_lieu, nguong_diem=0.7):
    ...
    Returns:
        dict: Dictionary chứa kết quả tìm kiếm và trạng thái thành công.
              Ví dụ: {'success': 1, 'results': [{'MaKMCP': ..., 'TenKMCP': ...}]}
    """
    # Bước 1: Trích xuất các chuỗi văn bản từ DataFrame
    # Lấy cột TenKMCP từ DataFrame
    cac_chuoi_mau = danh_sach_du_lieu['TenKMCP'].tolist()
    ...
    for i, diem in enumerate(diem_tuong_dong[0]):
        if diem >= nguong_diem:
            ...
            ket_qua_phu_hop.append(danh_sach_du_lieu.iloc[i].to_dict())
