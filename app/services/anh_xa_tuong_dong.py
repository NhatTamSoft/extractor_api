...
# Khởi tạo mô hình GLOBALLY hoặc PASS nó vào hàm
# Điều này đảm bảo mô hình chỉ được tải VÀ KHỞI TẠO MỘT LẦN DUY NHẤT
device = 'cuda' if torch.cuda.is_available() else 'cpu'
print(f"Sử dụng thiết bị: {device}")
# Tải mô hình ra ngoài hàm để tránh tải lại nhiều lần
try:
    model = SentenceTransformer('dangvantuan/vietnamese-embedding', device=device)
    print(f"Đã tải mô hình 'dangvantuan/vietnamese-embedding' thành công.")
except Exception as e:
    print(f"Lỗi khi tải mô hình: {e}. Đảm bảo bạn có kết nối internet hoặc mô hình đã được tải xuống trước đó.")
    # Fallback to a smaller, more generic model if the specific one fails
    # This might not be ideal for Vietnamese text, but ensures the code runs
    model = SentenceTransformer('all-MiniLM-L6-v2', device=device)
    print("Sử dụng mô hình 'all-MiniLM-L6-v2' thay thế.")
...
def tim_kiem_tuong_dong(chuoi_can_tim, danh_sach_du_lieu, nguong_diem=0.7):
    ...
    Args:
        ...
        model (SentenceTransformer): Đối tượng mô hình SentenceTransformer đã được khởi tạo.
    ...
    """
    # Kiểm tra và chuyển đổi chuoi_can_tim thành string nếu cần
    if not isinstance(chuoi_can_tim, str):
        try:
            chuoi_can_tim = str(chuoi_can_tim)
        except Exception as e:
            return {
                'success': 0,
                'results': [],
                'message': f'Không thể chuyển đổi chuoi_can_tim thành string: {str(e)}'
            }

    # Bước 1: Mô hình đã được khởi tạo và truyền vào, không cần khởi tạo lại ở đây
    ...
    if ket_qua_phu_hop:
        return {
            'success': 1,
            'results': [ket_qua_phu_hop[0]] if ket_qua_phu_hop else [] # Chỉ trả về kết quả tốt nhất
        }
    else:
        highest_score = 0.0
        if diem_tuong_dong.numel() > 0:
...
if __name__ == "__main__":
    ...
    du_lieu_can_tim = pd.DataFrame([
        ...
        {
            "MaKMCP": "CP7", # Thêm MaKMCP để khớp với định dạng du_lieu_mau_df, dù không dùng đến trong du_lieu_can_tim
            "TenKMCP": "Chi phí dự phòng"
        },
        {
        ...
        {
            "MaKMCP": "CP6", # Thêm MaKMCP để khớp với định dạng du_lieu_mau_df, dù không dùng đến trong du_lieu_can_tim
            "TenKMCP": "Chi phí khác"
        },
        {
            "TenKMCP": "Chi phí khảo sát địa chất"
        ...
        {
            "MaKMCP": "CP3", # Thêm MaKMCP để khớp với định dạng du_lieu_mau_df, dù không dùng đến trong du_lieu_can_tim
            "TenKMCP": "Chi phí thiết bị"
        },
        {
        ...
        {
            "MaKMCP": "CP2", # Thêm MaKMCP để khớp với định dạng du_lieu_mau_df, dù không dùng đến trong du_lieu_can_tim
            "TenKMCP": "Chi phí xây dựng"
        },
        {
    ...
    for index, row in du_lieu_can_tim.iterrows():
        ...
        # Truyền đối tượng model đã được khởi tạo vào hàm
        ket_qua = tim_kiem_tuong_dong(chuoi_can_tim, du_lieu_mau_df, nguong_diem=0.5)
        if ket_qua['success']:
